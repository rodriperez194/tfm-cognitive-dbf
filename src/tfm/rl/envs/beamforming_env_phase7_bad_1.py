...
class BeamformingEnvPhase7(gym.Env):
    ...
    3. Only when both constraints are satisfied is the normalized SINR used as
       the reward. Otherwise, a fixed failure penalty is returned.

    For zero-jammer episodes, the jammer condition is automatically satisfied.
    Instantaneous MVDR is retained only as an evaluation reference and does not
    ...
    def _compute_threshold_reward(
        ...
        """Compute the Phase 7 threshold-gated reward."""
        ...
        soi_constraint_violation_db = max(
            0.0,
            float(soi_gain_loss_db) - self.reward_soi_max_gain_loss_db,
        ) if np.isfinite(soi_gain_loss_db) else self.max_soi_gain_loss_db
