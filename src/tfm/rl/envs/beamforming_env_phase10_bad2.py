...
class BeamformingEnvPhase10(gym.Env):
    ...
    Reward
    ------
    This independent Phase 10 variant keeps the SOI constraint as a hard gate
    but makes the jammer constraint soft for learning:

    1. Invalid direct actions, numerical errors and SOI-gain violations keep
       the same hard penalties as the original Phase 10 environment.
    2. The configured mean jammer-leakage threshold is still evaluated and
       reported, but exceeding it no longer replaces the reward with the same
       fixed failure value.
    3. Once the SOI constraint is satisfied, the normalized SINR remains part
       of the reward even when the jammer threshold is violated.
    4. The dense jammer-leakage penalty is also evaluated in that soft-violation
       region:

           penalty_j = weight_j * clip(
               mean_leakage / leakage_scale,
               0,
               leakage_clip,
           )

    5. Stepped bonuses are applied only when both the SOI and jammer diagnostic
       conditions are satisfied, preserving the original valid-region reward.

    Therefore, the jammer threshold remains useful as a diagnostic success
    criterion, while SAC receives a graded learning signal on both sides of the
    threshold instead of a flat -10 reward throughout the jammer-failure
    region. Mean jammer leakage is intentionally retained in this ablation;
    changing to total jammer leakage is a separate experiment.

    A phase-invariant teacher-similarity component can also be added.

    The deterministic teacher follows the same update cadence as the agent.
    It is computed once from the geometry observed at the beginning of each
    control block and then held fixed for ``weight_hold_steps`` physical steps.
    Therefore, K=1 preserves the original instantaneous behavior, while K=5
    and K=20 keep both the agent weights and teacher weights unchanged during
    their respective five- and twenty-step control blocks. Physical metrics
    and the instantaneous MVDR reference are still recomputed at every
    physical substep using the current scenario geometry.

    Optional stepped bonuses can be configured independently for:

    - SINR loss relative to the MVDR reference (lower is better).
    - SOI gain loss relative to conventional steering (lower is better).
    - Mean jammer leakage (lower is better).
    - Teacher-weight similarity (higher is better).

    Each stepped-bonus configuration is an array-like object with two columns:

        [[threshold_1, bonus_1],
         [threshold_2, bonus_2],
         ...]

    For lower-is-better metrics, a row is satisfied when metric <= threshold.
    For teacher similarity, a row is satisfied when metric >= threshold.
    When several rows are satisfied, the largest configured bonus is applied;
    bonuses within the same component are not accumulated. Bonuses from
    different components are accumulated.

    For zero-jammer episodes, the jammer condition is automatically satisfied
    and the jammer bonus is zero. Instantaneous MVDR is retained only as an
    evaluation reference and does not participate directly in the base reward.
    """
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None):
        ...
        info = {
            ...
            "reward_jammer_gate_mode": "soft_diagnostic",
            "reward_sinr_loss_bonus_steps": (
    ...
    def _compute_threshold_reward(
        ...
        """
        Compute Phase 10 reward with a hard SOI gate and soft jammer gate.

        Hard failures are preserved for:

        - invalid direct-weight actions,
        - numerical errors or non-finite SINR,
        - SOI gain-loss violations.

        The jammer threshold remains a diagnostic condition. When it is
        violated while the SOI remains valid, the environment does not return
        the fixed failure penalty. Instead, it keeps a continuous objective:
        ...
        This gives the agent a graded signal in the jammer-failure region.
        Stepped bonuses remain restricted to transitions satisfying both the
        SOI and jammer diagnostic conditions, so the original reward in the
        valid region is preserved.
        """
        ...
        valid_objective = 0.0
        clipped_valid_objective = 0.0
        teacher_reward_component = 0.0
        soft_jammer_violation = False
        ...
        if invalid_direct_action:
            ...
            teacher_reward_component = float(
                self.reward_teacher_similarity_weight
            ...
            if not soi_condition_met:
                ...
                if self.num_active_jammers > 0:
                    if np.isfinite(jammer_mean_leakage):
                ...
                if jammer_condition_met:
                    ...
                    soft_jammer_violation = True
                    reward = float(
                        clipped_valid_objective
                        + teacher_reward_component
                        + jammer_leakage_reward_component
                    )
                    failure_reason = "jammer_constraint_soft"
                    failure_applied = False
        ...
        info = {
            ...
            "reward_soft_jammer_violation": bool(soft_jammer_violation),
            "reward_jammer_gate_mode": "soft_diagnostic",
