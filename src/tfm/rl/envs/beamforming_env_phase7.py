...
class BeamformingEnvPhase7(gym.Env):
    """
    Gymnasium environment for Phase 7 direct complex-weight beamforming.

    The observation architecture is unchanged from the previous phases. The
    agent observes the SOI direction and up to three jammer directions, using
    either normalized angles or 3D unit vectors with jammer-presence masks.

    The action is a direct real/imaginary parameterization of the complete
    complex beamforming vector:

        action = [Re(w_1), ..., Re(w_E), Im(w_1), ..., Im(w_E)]

    where E = array.N * array.M and every action component belongs to [-1, 1].
    The action is not interpreted as a residual correction. Power normalization
    is delegated to ``Phased_Array_NB.set_weights``. With
    ...
    Reward
    ------
    The reward follows a threshold-gated approach inspired by DRLNuller:

    1. The SOI gain loss relative to conventional steering must remain below a
       configured threshold.
    2. For one or more active jammers, the mean jammer-to-SOI gain leakage must
       remain below a configured threshold.
    3. Only when both constraints are satisfied is the normalized SINR used as
       the base reward. Otherwise, a fixed failure penalty is returned.
    4. For valid jammer-present transitions, an optional dense penalty is
       subtracted according to the normalized mean jammer leakage:
       ...
       This term is zero when no jammer is active or when its configured weight
       is zero.
    ...
    def step(self, action: np.ndarray):
        """Apply one direct complex-weight action for one control block."""
    ...
    def _validate_configuration(self) -> None:
        ...
        if self.complex_weight_mode != "real_imag":
            raise ValueError(
                "Phase 7 direct control currently requires "
    ...
    def _sample_valid_scenario(self) -> dict:
        ...
        if last_error is not None:
            raise RuntimeError(
                "Could not sample a valid Phase 7 direct-weight scenario."
            ) from last_error

        raise RuntimeError("Could not sample a valid Phase 7 direct-weight scenario.")
    ...
    def _action_to_direct_weights(
        ...
        """Convert a 2E real/imaginary action into direct complex weights."""
        ...
        weights_flat = real_action + 1j * imag_action
    ...
    def _build_invalid_action_info(self) -> dict:
        """Build logging information for an invalid direct action."""

        return {
            "raw_action": np.zeros(self.action_dim, dtype=float),
            "direct_real_action": np.zeros(self.num_elements, dtype=float),
            "direct_imag_action": np.zeros(self.num_elements, dtype=float),
            "direct_weights_before_normalization": np.zeros(
                (self.array.N, self.array.M), dtype=np.complex128
            ),
    ...
    def _build_block_info(
        ...
        """Build aggregated information for one Phase 7 control block."""
