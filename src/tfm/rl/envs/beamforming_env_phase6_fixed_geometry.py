...
class BeamformingEnvPhase5(gym.Env):
    ...
    def __init__(
        ...
        fixed_desired_source_position_m: np.ndarray | list[float] | tuple[float, float, float] | None = None,
        fixed_jammer_positions_m: np.ndarray | list | tuple | None = None,
        ...
        self.fixed_desired_source_position_m = self._normalize_optional_position_m(
            fixed_desired_source_position_m,
            name="fixed_desired_source_position_m",
        )
        self.fixed_jammer_positions_m = self._normalize_optional_position_list_m(
            fixed_jammer_positions_m,
            name="fixed_jammer_positions_m",
        )
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None):
        ...
        info = {
            ...
            "fixed_desired_source_position_m": (
                None
                if self.fixed_desired_source_position_m is None
                else self.fixed_desired_source_position_m.copy()
            ),
            "fixed_jammer_positions_m": (
                None
                if self.fixed_jammer_positions_m is None
                else self.fixed_jammer_positions_m.copy()
            ),
    ...
    def _validate_configuration(self) -> None:
        ...
        if self.fixed_desired_source_position_m is not None:
            fixed_soi_distance = float(
                np.linalg.norm(
                    self.fixed_desired_source_position_m - self.array_position
                )
            )

            if fixed_soi_distance < self.min_source_distance_m:
                raise ValueError(
                    "fixed_desired_source_position_m is closer to the array "
                    "than min_source_distance_m."
                )

        if self.fixed_jammer_positions_m is not None:
            if self.fixed_jammer_positions_m.shape[0] == 0:
                raise ValueError("fixed_jammer_positions_m cannot be empty.")

            required_fixed_jammers = self.num_active_jammers

            if self.active_jammers_choices is not None:
                required_fixed_jammers = max(self.active_jammers_choices)

            if self.fixed_jammer_positions_m.shape[0] < required_fixed_jammers:
                raise ValueError(
                    "fixed_jammer_positions_m must contain at least one "
                    "position for every active jammer that can be used."
                )

            for fixed_jammer_position in self.fixed_jammer_positions_m:
                fixed_jammer_distance = float(
                    np.linalg.norm(fixed_jammer_position - self.array_position)
                )

                if fixed_jammer_distance < self.min_source_distance_m:
                    raise ValueError(
                        "Every fixed jammer position must be farther from the "
                        "array than min_source_distance_m."
                    )
    ...
    # ============================================================
    # Optional fixed geometry helpers
    # ============================================================

    @staticmethod
    def _normalize_optional_position_m(
        position_m,
        name: str,
    ) -> np.ndarray | None:
        """
        Normalize an optional fixed 3D position.
        """

        if position_m is None:
            return None

        position = np.asarray(position_m, dtype=float).reshape(-1)
    ...
    @staticmethod
    def _normalize_optional_position_list_m(
        positions_m,
        name: str,
    ) -> np.ndarray | None:
        """
        Normalize optional fixed 3D positions.
        ...
        if positions.ndim == 1:
            if positions.size != 3:
                raise ValueError(
                    f"{name} must be either one 3D position or an array "
                    "with shape (num_positions, 3)."
                )
            ...
            if positions.shape[1] != 3:
                raise ValueError(f"{name} must have shape (num_positions, 3).")
    ...
    def _sample_desired_source_position_xyz(self) -> np.ndarray:
        """
        Sample or return the desired source position.

        If fixed_desired_source_position_m is provided, the SOI position is
        fixed. Otherwise, the original random sampling logic is used.
        """

        if self.fixed_desired_source_position_m is not None:
            return self.fixed_desired_source_position_m.copy()

        return self._sample_source_position_xyz()

    def _sample_jammer_source_position_xyz(self, jammer_idx: int) -> np.ndarray:
        """
        Sample or return one jammer source position.

        If fixed_jammer_positions_m is provided, jammer_idx selects the fixed
        position. Otherwise, the original random sampling logic is used.
        """

        jammer_idx = int(jammer_idx)

        if self.fixed_jammer_positions_m is not None:
            if jammer_idx >= self.fixed_jammer_positions_m.shape[0]:
                raise ValueError(
                    "fixed_jammer_positions_m does not contain enough "
                    "positions for the requested active jammer count."
                )

            return self.fixed_jammer_positions_m[jammer_idx].copy()
    ...
    def _sample_valid_scenario(self) -> dict:
        ...
        for _ in range(self.max_scenario_sampling_attempts):
            try:
                desired_source_position = self._sample_desired_source_position_xyz()
    ...
    def _build_random_jammers_for_episode(
        ...
        for jammer_idx in range(self.num_active_jammers):
            ...
            jammer_position = self._sample_jammer_source_position_xyz(
                jammer_idx=jammer_idx,
            )
            x0, y0, z0 = jammer_position
            ...
            while attempts < 100:
                ...
                if self.fixed_jammer_positions_m is not None:
                    raise RuntimeError(
                        "A fixed jammer position violates "
                        "min_target_jammer_separation_deg with respect to "
                        "the current SOI position."
                    )
